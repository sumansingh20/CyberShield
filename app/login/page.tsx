...
export default function LoginPage() {
  ...
  // Quick login keyboard shortcut for development
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      const handleKeyPress = (e: KeyboardEvent) => {
        // Press Ctrl + Shift + L for quick login
        if (e.ctrlKey && e.shiftKey && e.key === 'L') {
          ...
          toast({
            title: "Quick Login Applied",
            description: "Credentials filled automatically",
          })
        }
      }
    ...
    try {
      ...
      if (response && response.success) {
        if (response.requiresTwoFactor) {
          ...
          toast({
            title: "Welcome back! 🎉",
            description: `Successfully logged in as ${response.user.firstName || (response.user.username?.includes('_') ? response.user.username.split('_')[0] : response.user.username)}`,
          })
  ...
  const handleTwoFactorSuccess = (token: string, user: any) => {
    ...
    toast({
      title: "Welcome back! 🎉",
      description: `Successfully logged in with 2FA as ${user.firstName || (user.username?.includes('_') ? user.username.split('_')[0] : user.username)}`,
    })
    router.push("/dashboard")
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-2 sm:p-4">
      ...
      <Card className="w-full max-w-sm sm:max-w-md relative z-10 shadow-2xl border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl mx-2">
        ...
        <CardContent className="px-4 sm:px-6">
          ...
          {/* Quick Login Demo (Development Only) - Hidden but functional */}
          ...
          <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
            <div className="text-xs text-gray-600 dark:text-gray-400 text-center">
              🔒 Secured with 256-bit encryption • 2FA Protected • SOC2 Compliant
            </div>
          </div>
        </CardContent>
      </Card>
